...
| H3: macOS Python and its bootstrap installer missed security fixes | Pin the authenticated PSF Python 3.13.15 installer and pip 26.2.1 wheel. Build without ensurepip, execute only the hash-verified pip wheel, isolate package configuration, require binary/hash-locked third-party wheels, and omit ensurepip from the shipped runtime. |
| H1: macOS Python caches accepted unauthenticated output | Always extract the runtime again from its pinned installer and rebuild site-packages from hashed wheels. Reject arbitrary Python.framework overrides. Regression tests seed modified frameworks and unexpected cached startup hooks and verify that they do not reach the output. |
| H2: native macOS inputs were fingerprinted without publisher authentication | Pin Samba 4.24.7 and the complete 21-bottle Homebrew dependency set recorded for its arm64 Sonoma build. Extract verified archives into a fresh temporary tree, resolve libraries only within that tree, reject local overrides, and remove external run paths. Retain the final app ZIP, all-file hashes, and input/build provenance in CI. Intel/universal packaging fails until an equivalent graph is reviewed. |
| H4: sshpass 1.10 mishandled failed reads | Build from its pinned source with a small read-error/EOF patch, and reject missing environment passwords without a null dereference. Run six parser sanitizer cases, missing-password handling, and real pseudo-terminal password delivery against the built program. No device or actual credentials are used. |
