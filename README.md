...
5. Enter the device’s administrator password and click **Save Device**. If SSH is closed, the app asks before enabling SSH and rebooting. First isolate the network: connect the Mac to a LAN Ethernet port, turn off its Wi-Fi, disconnect the Time Capsule’s WAN and other Ethernet clients, and disconnect all other Wi-Fi clients. Legacy ACP exposes a recoverable administrator password; the confirmation grants a temporary exception only for this device’s SSH enablement and reboot. No Terminal command or environment override is needed.
6. When prompted, verify the displayed **SSH fingerprint** against a trusted record or establish the device’s identity through the direct, isolated connection. Click **Trust verified device** only after verification. The app rechecks the fingerprint, saves the key locally, authenticates over strict SSH, and saves the profile. It refuses to replace an existing trusted key. Saving the device does not install Samba. After the profile is saved successfully, you can reconnect the normal network; if the device’s IP changes, verify the identity at the new address. If port 5009 is unreachable, check the current LAN IP in AirPort Utility and avoid guest networks; no ACP password is sent when that preflight fails.
