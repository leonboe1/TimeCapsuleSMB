...
"""
    ...
    def test_probe_managed_rsync_accepts_disabled_daemon_with_persistent_payload(self) -> None:
        ...
        remote_cmd = run_ssh_mock.call_args.args[1]
        self.assertNotIn('persistent rsync binary is missing', remote_cmd)
    ...
    def test_format_deployment_plan_contains_concrete_actions(self) -> None:
        ...
        plan = build_deployment_plan("root@10.0.0.2", paths, Path("bin/smbd"), Path("bin/mdns"), Path("bin/nbns"), service_path=Path("bin/service"))
    ...
    def test_enabled_rsync_plan_is_rejected(self) -> None:
        with self.assertRaisesRegex(ValueError, "removed from this fork"):
            build_deployment_plan("host", self._payload_home("/Volumes/dk2", "samba4"), Path("smbd"), Path("mdns"), Path("nbns"), service_path=Path("service"), rsync_enabled=True)
